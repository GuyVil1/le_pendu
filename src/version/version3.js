...
function game(){
    ...
    function difficulté(){
        do{
            level = prompt(`Vous voulez trouver un mot de \n1. 5 lettres? \n 2. 6 lettres? \n 3. 7 lettres?\n4. 8 lettres?\n 5. 9 lettres? \n 6. 10 lettres?\n entrez le chiffre correspondant à la difficulté choisie:`);
 ...
 **************************************/
 ...
 function newGame(difficulté){
    if(difficulté == 5){
        ...
        index = parseInt(Math.random() * (maximum - 0) + 0);
        mot = mots_7[index];
    }else if(difficulté == 8){
        let maximum = mots_8.length;
        index = parseInt(Math.random() * (maximum - 0) + 0);
        mot = mots_8[index];
    }else if(difficulté == 9){
        let maximum = mots_9.length;
        index = parseInt(Math.random() * (maximum - 0) + 0);
        mot = mots_9[index];
    }else if(difficulté == 10){
